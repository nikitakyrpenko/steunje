    ...
    <table cellspacing="0" cellpadding="0" border="0" align="center" width="100%" class="admTable">        
        <tr>
            <td class="admNavPanelFont" align="center">                
                <xsl:value-of select="java:getString($dict_categories, 'SELECT_CATEGORY_OF_MODULE')"/>&#160;<xsl:value-of select="@module-title"/>                
            </td>
        </tr>
        <tr>
        ...
        <th class="admTableTD">
            <a class="admAnchor">
                <xsl:attribute name="href">
                    <xsl:text>?command=get-list-command</xsl:text>&amp;listId=<xsl:value-of select="@id"/>
                </xsl:attribute>
