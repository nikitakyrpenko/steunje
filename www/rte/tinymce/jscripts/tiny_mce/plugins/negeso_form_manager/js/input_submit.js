var InputSubmitDialog = {
	...
	init : function(ed) {
		...
		// Check action
		if (elm != null && elm.nodeName == 'INPUT' && ('' + dom.getAttrib(elm,'type')) == 'submit')
			action = 'update';
...
function insertField() {
	...
	if (elm != null && elm.nodeName == "INPUT") {
		...
		html += makeAttrib('onclick','try {if (!validate(this.form)) return false;} catch(e) {}');
		if (tinyMCE.isOpera)
...
tinyMCEPopup.onInit.add(InputSubmitDialog.init, InputSubmitDialog);
