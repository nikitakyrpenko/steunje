...
var headerPlusFooterSize = -1; 
var accurateHeight = -1;
...
var Book_v5 = {
    ...
    ready: function(){
	   ...
	   if (config['minus_header_footer_area'] == "true") {
			headerPlusFooterSize = Number(config['header_plus_footer_size']);
	   }
	   ...
	   if (config['accurateHeight']) {
		   accurateHeight = Number(config['accurateHeight']);
	   }
        ...
        if(IS_AS_TEMPLATE=="true"){
           var height=$(window).height()+"px";
        	...
        	if( $('#fb5').hasClass('fullScreen') ){
              var height=$(window).height()+"px";
        	}else if (accurateHeight>=0) {
        		height = accurateHeight + "px";
        	}else if (headerPlusFooterSize >=0) {
				height=$(window).height()-this.toolsHeight-headerPlusFooterSize+"px" ;
           }else{           
              height=(width_book*HEIGHT_BOOK/WIDTH_BOOK)+this.toolsHeight+"px";
           }
