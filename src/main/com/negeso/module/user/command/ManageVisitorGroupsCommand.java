/*
 * @(#)ManageVisitorGroupsCommand.java  Created on 17.06.2004
 *
...
public class ManageVisitorGroupsCommand extends AbstractCommand {
    ...
    private static Logger logger =
        Logger.getLogger(ManageVisitorGroupsCommand.class);
...
//          TODO If we have user input, show it
            ...
            try {
                ...
                Xbuilder.setAttr(elGroup, "role", group.getRoleId());
                elGroup.appendChild(buildContainers(group));
                elGroup.appendChild(buildVisitors(group));
            ...
            elGroup.appendChild(buildVisitors(null));
            elGroup.appendChild(buildRoles());
            logger.debug("-");
        }
        return elGroup;
    ...
    private Element buildVisitors(Group group){
        logger.debug("+");
        Element elUsers = Xbuilder.createEl(elPage, "users", null);
        try {
            if(group != null){
                Iterator iter = group.getVisitiors().iterator();
                while (iter.hasNext()) {
                    String[] info = (String[]) iter.next();
                ...
                Iterator iter = User.getVisitors().iterator();
                while (iter.hasNext()) {
    ...
    /**
     * Include only role 'visitor' on the list of available roles
     */
    private Element buildRoles(){
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if("visitor".equals(role.getId())){
                    Element elRole = Xbuilder.addEl(elRoles, "role", null);
                    elRole.setAttribute("id", "visitor");
    ...
    private void delete() {
        ...
        try {
            Group group = new Group();
            group.load(id);
            logger.info("Group " + group.getName() + " id# " + group.getId() +
    ...
    private Element buildList() {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if(!"visitor".equals(group.getRoleId())){
                    continue;
                }
                Element elGroup =
