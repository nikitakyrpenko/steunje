var InputCheckboxDialog = {
	...
	init : function(ed) {
		...
		// Check action
		if (elm != null && elm.nodeName == 'INPUT' && ('' + dom.getAttrib(elm,'type')) == 'checkbox')
			action = 'update';
		...
		if (action == "update") {
			...
			formObj.check.checked    = ('' + dom.getAttrib(elm, 'checked') == 'true' || dom.getAttrib(elm, 'checked') == 'checked');
		}
	},
...
function insertField() {
	...
	if (elm != null && elm.nodeName == "INPUT") {
		...
		if (tinyMCE.isOpera)
			html += makeAttrib('class', 'form_checkbox');
		html += " />";
...
tinyMCEPopup.onInit.add(InputCheckboxDialog.init, InputCheckboxDialog);