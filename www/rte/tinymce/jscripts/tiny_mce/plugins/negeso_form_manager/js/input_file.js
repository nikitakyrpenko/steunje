var InputFileDialog = {
	...
	init : function(ed) {
		...
		// Check action
		if (elm != null && elm.nodeName == 'INPUT' && ('' + dom.getAttrib(elm,'type')) == 'file')
			action = 'update';
		...
		if (action == "update") {
			...
			formObj.required.checked = ('' + dom.getAttrib(elm, 'required') == 'true' || dom.getAttrib(elm, 'required') == 'required');
		}
	},
...
function insertField() {
	...
	if (elm != null && elm.nodeName == "INPUT") {
		...
		setAttrib(elm, 'required', formObj.elements['required'].checked == true ? 'true' : '');
...
tinyMCEPopup.onInit.add(InputFileDialog.init, InputFileDialog);